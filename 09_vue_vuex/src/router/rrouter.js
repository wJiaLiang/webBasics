...
import Document from '@/components/routerCom/document'
import noFindele from '@/components/routerCom/404'
import slider from '@/components/routerCom/slider'

import User from '@/components/routerCom/user'
import login from '@/components/routerCom/login'
import Plugin from '@/components/routerCom/Plugin'
import study from '@/components/routerCom/study'
import work from '@/components/routerCom/work'
import hobby from '@/components/routerCom/hobby'
...
let rrouter = [
    {
      ...
      name: 'Home',
      component: Home,
      alias: '/routerCom/index',   // 别名，当访问 index的时候就能够匹配到home组件; 注意点,路由可以匹配到，但是router-link中用的home 就匹配不到了
      // meta 就相当于自定义的一些字段,
      meta: {
        index: 1,
        title: '主要'
    ...
    // ?表示可以匹配0次;既 /user 也算配配到 将URL后面的动态参数给 userId,然后再组件中获取;
    {
      ...
      path: '/routerCom/user/:tip?/:userId?',  // 可以多级  /user/vip/1   /user/vip/2   /user 
      name: 'User',
      component: User,
      meta: {
        index: 4,
